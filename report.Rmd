...
model_3 <- run_model(independent, dependent3, tot.votes, candidate3, input_slider, racename)
...
cat('First, we compare predictions from three different models for', paste0("params$candidate",i), '\n')
cat('vote share given demographic and total vote data' , '\n')  
kable(paste0("model_",i)$ei.table)
# cat('\\subsection{$Goodmans Ecological Regression$}')
cat('Next, we plot votes for', paste0("params$candidate",i), 'by the proportion of the population', '\n')
cat(params$racename, "according to Goodman's regression predictions", '\n')
cat("We use the following equation: ", '\n')
withMathJax(HTML(paste(
                 paste0("params$dependent",i),"=\\(\\LARGE\\beta_0 + \\beta_1\\)",params$independent, ".", "<br/><br/>Note that \\(\\LARGE\\beta_0 = \\)",
                 round(paste0("model_",i)$ei.table[1,3],3), "and \\(\\LARGE \\beta_1 =\\)", round(paste0("model_",i)$ei.table[2,3]-paste0("model_",i)$ei.table[1,3],3),".", "<br/><br/>")))
plot(paste0("model_",i)$gr.plot)
cat('\\subsection{$Ecological Inference$}')
cat("Finally, we calculate ecological inference predictions for", paste0("params$candidate",i), "\n")
cat("'s vote share and plot them with credible intervals. If the intervals overlap,", "\n")
cat("we cannot conclude that there was racially polarized voting for", paste0("params$candidate",i), ". \n")
plot(paste0("model_",i)$ei.plot)
cat("\n")
}
```
...
```{r echo = F}
      withMathJax(HTML(paste(
                 ...
                 round(model_1$ei.table[1,3],3), "and \\(\\LARGE \\beta_1 =\\)", round(model_1$ei.table[2,3]-model_1$ei.table[1,3],3),".", "<br/><br/>")))
```
...
```{r echo = F}
      withMathJax(HTML(paste(params$dependent2,"=\\(\\LARGE\\beta_0 + \\beta_1\\)",params$independent, ".", "<br/><br/> Note that \\(\\LARGE\\beta_0 = \\)",
                 round(model_2$ei.table[1,3],3), "and \\(\\LARGE \\beta_1 =\\)", round(model_2$ei.table[2,3]-model_2$ei.table[1,3],3),".", "<br/> <br/>")))
```
